...
Delta = chain_net(n); %Chain network
...
cyc_counter = 0;
chain_fig = figure;%('Position', [0, 0, 1365, 690]);
...
%{
print(chain_fig1,'-dpng','-r80',...
    sprintf('chain_n%d_eta%.2f_lambda%.2f_gamma%.1f__.jpg',...
    n,eta,lambda,gamma))
%}
...
%find concentrated and unique states in chain network
%how much do I have n-d

states_concentrated = NaN(ceil((n-d+1)/2),1);
...
for k = 1:size(states_concentrated,1)
    states_concentrated(k) = 2^(d+k-1)-2^(k-1)+1;
end
...
for k=1:size(states_concentrated,1)
    cyc_counter = cyc_counter + 1;
    p(k) = plot(y_t(:,states_concentrated(k))-1);
    hold on
    
    %legend
    leg{k} = dec2bin(states_concentrated(k)-1,n);
...
graph_title = strcat(sprintf('Chain: Concentrated shocks of size %d',d),...
    sprintf('\nParametrization:'),...
...
n,d,lambda*100,eta*100,gamma,mu*1000,sigma*10,delta*100,epsilon*10))
