%note: prob, 0011 and 1100 not taken out
...
%Find pairs of states which are identical up to rotation and mirror
    for i=1:size(aux,2)
        ...
        for j=1:size(aux,2)
            if i < j
                for k=1:n %may reduce to n-1
                    if isequal(aux(:,i), circshift(aux(:,j),k)) == 1 %simple rotation
                        state_pairs = [state_pairs; ...
                            aux_H_index(i),aux_H_index(j),k,NaN(1)];
                        flag = 1;
                    elseif isequal(aux(:,i), circshift(fliplr(aux(:,j)')',k)) == 1 %mirror image
                        state_pairs = [state_pairs; ...
                            aux_H_index(i),aux_H_index(j),NaN(1),k];
                        flag = 1;
                    end
                end
            end
        ...
        if flag == 0 % the state is unique
            state_pairs = [state_pairs; aux_H_index(i), NaN(1,3)];
        end
    end
    
    state_pairs_full = [state_pairs_full; state_pairs];
    %state_pairs_ = [state_pairs_; unique(state_pairs,'rows')]; %Matrix of state-pairs

end

state_pairs_ = unique(state_pairs_full(:,1:2),'rows');
...
%state_pairs_(isnan(state_pairs_(:,2)),1)
...
clearvars -except n state_pairs unique_states state_pairs01 unique_state_x_dist state_num_dist state_pairs_full
...
%%
a = state_pairs_full(:,3);
b = state_pairs_full(:,4);
...
a(find(isnan(state_pairs_full(:,2)))) = n;
b(find(isnan(state_pairs_full(:,2)))) = n;
